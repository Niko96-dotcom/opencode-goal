...
export function buildContinuationPrompt(state: GoalState, previousValidation?: ValidationResult) {
  ...
  const verification =
    state.verifyCommand ??
    "No shell verification command configured. Use concrete evidence from files, commands, artifacts, or logs."
  const previous = previousValidation
    ? summarizeValidation(previousValidation)
    : "No previous verification result in this run."
...
Checkpoint: ${currentTurn} of ${state.maxTurns}
...
- Before declaring completion, perform a completion audit against the objective and the evidence.
...
- If there is no defensible next action, required credentials are missing, or the verification surface cannot run, report BLOCKED.
